...
/**
 * This functions sets the radio ic for 100kSps operation. The property values are generator with WDS-Studio, so that all other necessary
 * parameters are set up aswell. To exit hdr(high data rate)-mode, the base config init funciton is called.
 * @param onoff If 0x00 radio ic base config will be loaded, if 0xff the high data rate mode settings will be loaded.
 */
void comm_hal_hdrmode(uint8_t onoff){
  if(onoff){
    si446x_set_property(0x20, 0x0C, 0x00, 0x03, 0x00, 0x07, 0x1E, 0x84, 0x80, 0x09, 0x8C, 0xBA, 0x80, 0x00, 0x07);
    si446x_set_property(0x20, 0x01, 0x0C, 0xE0);
